...
services:
  monitoring-manager:
    tty: true
    stdin_open: true
    image: {{ .Values.IMAGES_REGISTRY }}/cnrancher/monitoring-manager:v0.0.10
    volumes:
  ...
  prometheus-data:
    tty: true
    stdin_open: true
    image: {{ .Values.IMAGES_REGISTRY }}/cnrancher/prom-init:v0.0.1
    volumes:
      - prometheus-config:/etc/prometheus
      - prometheus-rule:/etc/prometheus-rules
      - prometheus-datas:/prometheus
  ...
  prometheus:
    tty: true
    stdin_open: true
    image: {{ .Values.IMAGES_REGISTRY }}/cnrancher/prometheus:v2.1.0
  ...
  alertmanager-data:
    tty: true
    stdin_open: true
    image: {{ .Values.IMAGES_REGISTRY }}/cnrancher/alertmanager-init:v0.0.1
    volumes:
      - alertmanager-config:/etc/alertmanager
  ...
  alertmanager:
    tty: true
    stdin_open: true
    image: {{ .Values.IMAGES_REGISTRY }}/cnrancher/alertmanager:v0.14.0
  ...
  graf-plugins:
    tty: true
    stdin_open: true
    image: {{ .Values.IMAGES_REGISTRY }}/cnrancher/grafana-plugins:1.0.1
    volumes:
      - grafana-datas:/var/lib/grafana
    labels:
      io.rancher.container.start_once: true
  ...
  grafana:
    ...
    image: {{ .Values.IMAGES_REGISTRY }}/cnrancher/grafana:6.5.2
